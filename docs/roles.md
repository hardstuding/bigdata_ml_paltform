...
| 作业模板 / CI-CD | 🟡 | **模板部分做了**(A 线第一步):4 个模板覆盖 hello / 批处理 ETL / 训练+注册 / 数据质量断言,`platform-submit --list-templates` 和 `--new` 可以直接生成。模板里写进了两件人容易漏的事——批处理**写完回查一次**、质量检查**不合格非零退出**。**CI-CD 那半没做**:作业代码还没有「提交到 git 就自动部署/测试」这条路 |
