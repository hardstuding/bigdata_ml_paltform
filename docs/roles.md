...
**平台今天对运维角色基本完整,对数据分析师大体可用(卡在"找数据"这一步),
对大数据开发和算法工程师是结构性缺失,对管理角色尚未开始。**

没部署的那 8 个组件不是零散遗漏——它们几乎精确地就是除分析师/运维之外
其它角色所依赖的东西。所以缺口是**按角色成块缺失**的,不是修修补补
就能补齐。

| 角色 | 能独立完成日常工作? | 主要卡点 |
|---|---|---|
| 运维 | ✅ 基本可以 | 缺统一控制面/Runbook;告警没有外部通知渠道 |
| 数据分析师 | 🟡 大体可以 | **第一步"找数据"就卡住**——OpenMetadata 没部署 |
| 大数据开发 | ❌ 不行 | 调度器起来了,它要调度的引擎(Spark/SeaTunnel/Kafka)全没部署 |
| 算法工程师 | ❌ 不行 | 入口(JupyterHub)和实验跟踪(MLflow)都没部署,只有末端 KServe 在 |
| 管理 | ❌ 不行 | 驾驶舱从未开始 |
...
**结论:**这个角色是目前完成度最高的。真实缺口是"告警送不到人"和
"排障知识没有 Runbook 化"——两个都不需要新组件,是打磨问题。
...
| **找数据(有哪些表)** | ❌ | **OpenMetadata 没部署**。这是链路第一步就断了——不知道有什么表,后面都无从谈起。目前只能靠问人或者直接 Trino `SHOW TABLES` 硬翻 |
| 申请表权限 | 🟡 | permission-request-app 已部署,分级审批链跑通(ADR-044/045)。但"浏览目录勾选表"这个体验(ADR-046)**依赖 OpenMetadata**,它没起来就退化回手打完整表名 |
...
**结论:**这个角色**只差一个 OpenMetadata**就能从"大体可用"变成"顺畅"。
它同时卡住"找数据"和"申请权限的好体验"两处,是当前性价比最高的一个动作。
...
| 环节 | 状态 | 说明 |
|---|---|---|
| 批量数据接入 | ❌ | **SeaTunnel 没部署**(ADR-011/037 设计和验证都做过) |
| 流式数据接入 | ❌ | **Kafka 没部署**(Strimzi KRaft,单独验证过健康,从没接进端到端管道) |
| 湖仓存储 | ✅ | Iceberg on MinIO + Hive Metastore(ADR-002) |
| 批处理引擎 | ❌ | **Spark Operator 没部署**(ADR-036 验证过真实读写 Iceberg) |
| 流处理引擎 | ❌ | Flink 只有角色设计(ADR-056),没有实现 |
| 调度 | ✅ | Airflow 已部署,DAG 单一源码 + CI 防漂移 |
| 作业可观测 | ❌ | **Spark History Server 没部署**(ADR-029) |
| 血缘 | ❌ | OpenMetadata 没部署;SeaTunnel 血缘(ADR-052)只验证过 API 机制;Spark 血缘(ADR-014)仅设计 |
| 数据质量 / 契约 | ❌ | 未开始(B 线) |
| 作业模板 / CI-CD | ❌ | 未开始(A 线) |

**结论:****这个角色今天基本什么都做不了。**调度器是唯一起来的东西,
但它要调度的三个引擎(Spark/SeaTunnel/Kafka)全部没部署。注意这些组件
**都单独验证通过过**,不是没做出来——是环境抽象没把它们在 cloud-full
上启用起来(ADR-057 第三批)。
...
| 环节 | 状态 | 说明 |
|---|---|---|
| **交互式开发(Notebook)** | ❌ | **JupyterHub 没部署**。即使拉起来,也没有"打开就自动连好 Trino、自动带自己权限"的体验——要自己装 client、自己填连接串(`docs/usage-guide.md` 已如实记录这个差距) |
...
| **实验跟踪 / 模型注册** | ❌ | **MLflow 没部署**。这是 ML 工作流的中枢,没有它,训练结果无处沉淀 |
...
优先级上,JupyterHub + MLflow 是解锁这个角色的最小组合。
...
2. **当前最高性价比的三个动作**(依据就是上面的表):
   - **部署 OpenMetadata** —— 一个动作同时解锁分析师的"找数据"、
     "浏览目录申请权限",以及治理的"数据资产盘点"
   - **部署 JupyterHub + MLflow** —— 算法工程师从"零"到"能开工"
   - **部署 Spark Operator + SeaTunnel** —— 大数据开发从"零"到"能开工"

   这三个动作都不是"开发新功能",都是**已经验证过的组件在 cloud-full
   上没启用**。所以真正该先做的是 ADR-057 第三批(环境抽象的组件选择层),
   做完这三个动作就从"人工 git mv 加祈祷"变成"改配置 + 渲染"。
