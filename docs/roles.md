...
| 作业模板 / CI-CD | 🟡 | **模板部分做完并接进 CI**(A 线第一步):4 个模板覆盖 hello / 批处理 ETL / 训练+注册 / 数据质量断言,`platform-submit --list-templates` 和 `--new` 直接生成;`scripts/check-job-examples.py` 在 CI 里查结构、语法、以及 **job.yaml 的字段对不对得上 `submit_job()` 的签名**(字段名写错在提交那一刻才报 TypeError,那时人已经在集群上了)。模板里写进了两件人容易漏的事——批处理**写完回查一次**、质量检查**不合格非零退出**。**CI-CD 那半仍没做**:作业代码还没有「提交到 git 就自动部署」这条路 |
...
| 推理可观测 | 🟡 | Grafana 能看到基础指标,没有面向模型的监控(延迟/漂移/调用量)。**2026-08-28 补记**:在这之前推理链路其实是断的(kserve-demo 不在 MinIO 白名单,见 ADR-080),而没有任何监控发现——**「没有面向模型的监控」的第一个代价不是看不到指标,是链路断了没人知道** |
| 灰度 / A-B | ❌ | KServe 原生 canary 没配置(设计判断见 architecture.md) |
| 模型审批 / 回滚 | ✅ | ADR-080,**2026-08-28 完整链路实机验证**:训练→注册→审批→部署→**真实推理请求返回预测 `[0,1]`**→回滚守卫。上线单位从「MinIO 里最新的目录」换成「注册表里被批准过的版本」——在这之前**连版本概念都没有**。回滚**只能回到批准过的版本**,否则回滚就成了绕过审批的后门。**灰度仍未做**(要先有「两个版本同时在线」的概念)|
