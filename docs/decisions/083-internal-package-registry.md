...
zhenghe:「公司内部自己开发的一些 python 包、java 包等,怎么共享和管理」。
...
| C. 复用公司已有的 Nexus / Artifactory | 如果有,这是最省事的 | **需要 zhenghe 确认公司有没有、以及网络通不通**。这条没被排除,只是现在没法验证——真有的话,换成它只需要改 pip 的 index 地址 |
