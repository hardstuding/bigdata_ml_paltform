...
| 环节 | 状态 | 验证级别 | 最后验证 | 证据 |
|---|---|---|---|---|
| 登录 / 统一入口 | ✅ | 集成验证 | 2026-09-02 | Keycloak SSO + platform-portal([ADR-047](../decisions/047-platform-portal.md))。**2026-08-30 实机验证按角色显示**:`data-analysts` 身份看不到 ArgoCD/Keycloak、看得到 SQL 工作台;`platform-team` 全部可见;三个自建应用都读得到 groups claim(此前 permission-request-app 读不到,导致组权限审批对所有人 403)。**2026-09-02 补上真正走浏览器/HTTP 的端到端 SSO 登录验证**:此前所有验证都是程序化的(impersonation 用 override_user、按角色显示用伪造请求头),**没有一次真的走完一遍登录**,于是 Superset 的 SSO 从 08-29 起就是坏的却没人发现(userinfo 相对路径少一段,FAB 把异常吞掉、静默回登录页)。修完实测:登录后落在 /superset/welcome/,用户自动建出来并按 platform-team 映射到 Admin | **2026-09-02 把这个验证扩到全部 10 个对人暴露的组件**(门户/Superset/Airflow/OpenMetadata/MLflow/Grafana/Argo Workflows/权限门户/建表/Spark History),全部真的带账号密码走完登录,10/10 通过 —— 也就是说 Superset 那次是个例,不是普遍现象。`scripts/52-verify-sso-login.sh <用户> <密码>` 可重复跑。**判据按组件形态分三类**:普通页面看落在哪、SPA(Airflow/Grafana)拿登录后的 cookie 请求一个需要认证的 API(不带 cookie 时是 401)、OpenMetadata 看重定向链里有没有发出 OM_SESSION 和 id_token(它的 token 在 URL fragment 里,curl 天然走不完最后一步) |
| 找数据(有哪些表) | ✅ | 集成验证 | 2026-08-23 | 采集自动发现,目录里 100+ 张表,字段和 Trino 真实表结构一致 |
| 申请表权限 | 🟡 | 集成验证 | 2026-09-03 | [ADR-044](../decisions/044-tiered-approval-workflow.md)/[045](../decisions/045-approval-backend-notifications-escalation.md)。**2026-09-03 第一次真的走完:**analyst001 提申请 → 按表的安全等级算出审批链 → 出现在 dev001 的「待我审批」里 → 批准 → 状态 `approved_pending_apply`。**🟡 是因为最后一步差一个凭据**:写回 `table-access-grants.csv` 需要 `GIT_TOKEN`(在人工凭据清单里登记着,没配时降级成提示人工加那一行 —— 这个降级路径使用指南里写明了)。**走这一遍连着挖出三个阻断**:目录里 200 张表带安全等级的是 0 张(→ `scripts/58-backfill-security-levels.sh`)、门户读的那份 employees.csv 是旧拷贝导致审批人算成一个不存在的账号、dev001/viewer001 在 Keycloak 里没建过 |
...
| 血缘 | ✅ | 集成验证 | 2026-09-02 | 直接查 lineage API 确认边存在。**Spark 血缘仍没落地**,而且 2026-08-30 核实发现 [ADR-014](../decisions/014-spark-lineage-official-agent.md) 选的那个 agent 只有 2024 年的 `1.0-beta`(Java 11 + OpenLineage 1.7),**不认 Spark 4** —— artifact 要重选 | **2026-09-02 发现这条能力其实已经坏了几天**:OpenMetadata 写血缘时要同步更新搜索索引,而它连OpenSearch 的 TLS 主机名校验过不去(镜像自带的 demo 证书 SAN 里没有集群内服务名),`PUT /api/v1/lineage` 直接 500。表现只是seatunnel_device_events 这条 DAG 的一个任务红了 —— **能力表这一格当时还是绿的**。修完重跑,DAG 三个任务全绿,血缘 API 查得到`airflow-platform.seatunnel_device_events → trino.iceberg.demo.device_events`([ADR-093](../decisions/093-opensearch-http-tls.md)) |
...
| 环节 | 状态 | 验证级别 | 最后验证 | 证据 |
|---|---|---|---|---|
| 交互式开发(Notebook) | ✅ | 集成验证 | 2026-09-03 | JupyterHub + 平台镜像自带 `platform_sdk`([ADR-058](../decisions/058-lightweight-developer-experience.md))。**身份不再是共享服务账号**:notebook 里查到的 `current_user` 是登录者本人,列级脱敏对他生效 | **2026-09-03 发现这一格从 08-29 起就是坏的**:`singleuser.extraEnv` 写成了 Kubernetes 那种 `- name:/value:` 列表,而 KubeSpawner 的 `environment` traitlet 要 dict —— **点「启动服务器」直接 500,谁都起不来**,而 hub 自己好好的、登录正常、ArgoCD 全绿。修完实测走完整条:起 pod → `platform_sdk.query()` 查数(`select current_user` 返回 algo001 本人,impersonation 生效)→ `submit_job()` 提交作业 Succeeded → Pod 上带着 `kueue.x-k8s.io/local-queue-name: algorithm-team`(按组配额生效)→ Workflow 上带着 `platform-sdk/submitted-by: algo001`。**同时补了两个缺口**:算法账号在授权表里一条 grant 都没有(查什么都是 PERMISSION_DENIED)、`MLFLOW_TRACKING_URI` 从没注入过(实验记录静默进了 pod 本地目录) | **2026-09-03 又发现一层**:`http_timeout`(等 jupyter server 回话)默认只有 30 秒,而 `startTimeout`(等 Pod Running)配的 300 管不到它 —— 连续三次 spawn,Pod 每次都 Running、每次都在 30 秒那一刻被判超时删掉。**对新用户这条能力实际是不可用的**,只有镜像和缓存都热了才偶尔成功。设成 180 后冷启动也能过。同轮验了内部包:`pip install platform-helpers` 直接装上 0.1.0 并能 import |
