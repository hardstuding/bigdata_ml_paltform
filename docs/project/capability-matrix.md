...
| 看板 / BI | 🟡 | Superset 接 Keycloak SSO 已验证;连 Trino 走 impersonation,**数据权限按登录用户判**(ADR-074)。**2026-08-29 修掉一个产品层的权限漏洞**:此前 `AUTH_USER_REGISTRATION_ROLE="Admin"` 且 OAuth scope 里没有 `groups` —— 任何能登录平台的人在 Superset 里都是管理员(能改数据源连接串、删别人的看板、进 Security 菜单)。现改成按 Keycloak 组映射角色(platform-team→Admin、data-analysts→Alpha+sql_lab、algorithm-team→Gamma+sql_lab、viewers→Gamma),未分组的人落 Gamma 而不是 Admin。**还是 🟡 因为没在集群上实测过登录后拿到的角色**,下次开机要验:用 analyst001 登录,确认拿到 Alpha 而不是 Admin |
