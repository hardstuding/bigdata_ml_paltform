...
## P1.5:外部评审(2026-08-29,Codex)提出的第二~四批
...
**验收标准里"底层组件不再对所有角色一视同仁地暴露"这条还没做** —— 工具
清单目前仍然对所有人显示同一份。这条要判断用户在哪个组,而门户现在拿不到
groups(和"代他人建表"卡在同一个地方:要给 Keycloak client 加 scope)。
...
**还没做**:字段说明、分区、生命周期、质量规则、提交前预览;以及"哪些角色
可直接建表、哪些要审批"(现在是任何登录用户都能建)。

**"代他人建表"被这次改动挡掉了**,这是有意的取舍:要放开就得判断"你是不是
平台组",而那需要 oauth2-proxy 传 access token + 给 Keycloak client 加
`groups` scope —— 加 scope 这件事在 MLflow 上炸过一次(client 没配这个
scope,Keycloak 直接 `invalid_scope`,登录页都进不去)。所以单独做、单独
实机验证,不顺手塞进这一轮。

### 审批体验 —— 2026-08-29 做完 6/7,**待实机验证**
...
| **续期** | ❌ **没做**。续期要改 grants.csv 里已有那行的 `expires_at`,而现在的 `apply_grant_to_git` 只会追加新行 —— 不是加个按钮的事,要动写入逻辑 |

**阻塞**:真实 HR/IdP 对接需要 zhenghe 提供对接方,占位组织数据不能标成
生产完成。

### 文档职责重构 —— 2026-08-29 基本做完
