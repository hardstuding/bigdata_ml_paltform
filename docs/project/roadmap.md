...
**依赖**:作业详情要 Argo 的 API,已有;权限数据要 permission-request-app 开
只读接口(现在没有)。
...
**验收**:状态中文化、时间按用户时区;申请理由按敏感等级决定是否必填;
批准/拒绝支持意见,拒绝原因必填;到期提醒、续期、催办;失败重试
(**这条 2026-08-29 已做**,见 `/internal/retry-pending-applies`)。
**阻塞**:真实 HR/IdP 对接需要 zhenghe 提供对接方,占位组织数据不能标成
生产完成。

### 文档职责重构

第一批已经做了目录分层(`docs/project/` 独立)和死链检查。**还没做的**:
`current-work.md` 收敛到一页;capability matrix 只留状态+最后验证时间+证据
链接;使用指南按角色和任务拆分,每节带前置条件/操作/预期结果/常见失败;
Runbook 每条统一成触发条件/影响/前置检查/操作/验证/回滚。
**验收**:同一能力不能在两份权威文档里同时是"已完成"和"未完成";每项能力
标注 demo / 集成验证 / 生产验证 / 计划中。
