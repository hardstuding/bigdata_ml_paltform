# 使用指南的旧版(存档,2026-08-29 前)

> **这不是权威文档,而且里面有几条是错的。** 现在看
> [`usage-guide.md`](usage-guide.md)。
>
> 留档只为可追溯。旧版里至少三处内容和 `capability-matrix.md` 直接矛盾,
> 其中一条是**反着说的**:它告诉用户"权限门户只做决策与留痕,不做真正的
> Trino 查询拦截",而 OPA 强制执行、列级脱敏、行级过滤在 2026-08-26 就
> 全部实机验证过了。一份告诉用户"没人拦你"的安全文档,比没有文档更糟。
>
> 另外两处:说 Trino 是"唯一的交互式 SQL 入口"(它连 SQL 编辑器都没有,
> 见 ADR-084),说 notebook 里"怎么免密拿到当前用户的 Trino 凭据没有定论"
> (2026-08-29 已经做完并实机验证)。
