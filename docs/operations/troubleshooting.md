...
**什么时候必须做**:这套东西从开发测试转向承载真实数据或真实用户时。
这是上生产的硬门禁之一,不是可选项。

**为什么不能只靠"改文档"**:凭据一旦以任何形式离开过它该在的地方
(写进文档、贴进工单、出现在日志里),**改文档是没用的** —— 有效的处置
只有换掉它本身。这一点对任何版本控制系统都成立:删掉当前版本不等于删掉
历史。

**开发测试阶段的取舍**:按既定安排不轮换,前提是这台机器只跑 demo 数据、
只有内部人访问。这个前提一旦变化,这条立刻生效。
