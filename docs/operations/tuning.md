...
| Keycloak 会话超时 | **`environments/resource-profiles.yaml`** 的 `keycloak_sso_session_idle_timeout` / `keycloak_sso_session_max_lifespan`(秒) | local-lite / cloud-full **28800/86400**(8/24 小时);prod **1800/28800**(0.5/8 小时) | **2026-08-30 更正**:这一行原来指向那个 Keycloak 配置脚本(**那是生成物**)、且说「cloud-full/prod 部署前必须重新评估」—— 实际这两个值已经按环境分档(ADR-059),prod 那档已经收紧到 30 分钟空闲 |
