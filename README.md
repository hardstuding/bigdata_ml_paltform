...
- ✅ OpenMetadata + OpenSearch:已验证 web 应用正常响应,也接了 Keycloak custom-oidc SSO(`/api/v1/auth/login` 跳转已验证,client_id/redirect_uri 都对)。Postgres 后端(不用默认 MySQL),采集编排用官方 k8s 模式(不用 Airflow,见 ADR)。踩了一个不小的坑:认证配置只在数据库首次初始化时从环境变量写入,之后以数据库为准,改 env var 对已有数据库不生效,这台机器上是重建了 openmetadata_db 才生效的(测试数据,详见 troubleshooting.md)
...
- 浏览器完整登录(不只是服务端跳转)待你在自己机器上加好 `/etc/hosts`(`argocd`/`grafana`/`trino`/`superset`/`openmetadata`.local-lite.test → 127.0.0.1)后自己试一遍
