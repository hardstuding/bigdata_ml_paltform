...
2026-08-30 讨论"在 OpenMetadata 里显示当前用户有哪些表权限"这个
需求时:
...
这个仓库要开源。**写进来的每一行都会被陌生人读到**,所以判据不只是"有没有
长期价值",还有"公开合不合适"。

2026-09-02 盘点发现三类不该在这里的东西:

| 类型 | 例子 | 处理 |
|---|---|---|
| **私人信息** | 本机路径 `/Users/<人名>/...`、另一个真实项目的名字、个人邮箱 | **删掉**,不留痕 |
| **会话流水** | "等某某醒了确认"、"现场看效果时发现"、"他当场指出" | 删掉,只留结论 |
| **人名 + 日期做主语** | "某某 2026-08-30 说要 X" | 改写成理由本身:"X,因为 Y" |

**第三类最容易被误解成"要全删"** —— 不是。那些注释里往往藏着**真正的技术
理由**,那是这个仓库最值钱的部分。要删的是"谁在哪天说的"这个包装,留下的是
"为什么这么做"。

对比:

    ✗  某某 2026-08-23 说:"这个你就按一般云服务的计算就行"
    ✓  成本核算按云服务商的公开单价折算,不接真实账单接口 ——
       这一档没有多租户计费需求,接账单 API 要额外的权限和凭据

后者信息量更大,而且**不依赖读者认识谁**。

**例外**:`docs/journal/` 是有意保留的时间线记录(踩坑过程本身有价值),
但同样不能有私人信息;人名统一用角色代称("平台负责人""使用方")。

**新写内容时的自检**:这句话如果被一个不认识我们的人读到,他能不能只靠
它本身理解?不能,就重写。
