...
- **公网 IP**:不固定,不是 EIP,每次重新开机可能变——需要连接时先
  `aliyun ecs DescribeInstances` 查当前实际 IP,不要相信这份文档里
  写死的旧值。
- **磁盘**:系统盘 40GiB(ESSD PL1)+ 数据盘 200GiB(ESSD PL0,
  2026-08-20 从 ESSD AutoPL 换过来的——AutoPL 按实际 IOPS/吞吐弹性
  计费,比固定性能档位贵不少,PL0 对这个 K8s 集群的负载够用。Aliyun
  不支持 AutoPL 原地降级成普通 ESSD,走的是"打快照→从快照建新盘→
  detach 旧盘/attach 新盘→验证数据完整→删旧盘"这条路径,不是简单的
  一个 API 调用,细节见 `docs/journal/2026-08.md` 当天的记录)。两块都
  设置了"不随实例释放"(`DeleteWithInstance=False`,迁移后手动确认过)
